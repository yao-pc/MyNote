...
.DS_Store
```
- - **重点**：`.gitignore` 文件很重要，它可以让 Git 忽略 `workspace.json` 这类本地配置文件。不同电脑的窗口布局不一样，如果不忽略这个文件，每次同步都容易产生冲突。
...
```
- 在执行 `git push` 时，终端会提示你登录。**用户名填你的 GitHub 用户名，密码不是 GitHub 登录密码，而是一个叫“个人访问令牌（Personal Access Token，简称 PAT）”的凭证**。如果还没生成 PAT，可以在 GitHub 设置里的 `Developer settings` -> `Personal access tokens` 下生成一个，记得勾选 `repo` 权限。
